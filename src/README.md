...
- `main.ts` — the single entry point. Wires Supabase auth (password, passkey, TOTP 2FA), optional `fiducia-auth` session verification, the API-key create/rotate flow, preferences, and security-session controls into the static app shell. Also drives two live-update channels: the customer API WS/SSE stream and Supabase realtime (`fiducia_*` table changes).
