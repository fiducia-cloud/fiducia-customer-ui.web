...
- `main.ts` — the single entry point. Wires Supabase auth (password, passkey, TOTP 2FA), optional `fiducia-auth` session verification, the API-key create/rotate flow, preferences, and security-session controls into the static app shell. The customer API WS/SSE stream is a non-sensitive refresh heartbeat; credential metadata is fetched through authenticated BFF routes.
